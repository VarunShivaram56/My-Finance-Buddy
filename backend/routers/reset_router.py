...
from database.models import Asset, Loan, NonBankingTransaction, Statement, Transaction, User
...
def reset_data(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> dict:
    ...
    db.query(Asset).filter(Asset.user_id == current_user.id).delete(synchronize_session=False)
    db.commit()
